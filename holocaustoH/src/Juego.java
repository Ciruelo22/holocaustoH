...
public class Juego {
		...
		for(int fil = -1; fil < Habitacion.ALTO; fil++) {
			...
			for(int col = 0; col <= Habitacion.ANCHO; col++) {
				...
				if(posObj != -1) {
					...
					if(objJ.getTipoObjeto() == ObjetoJuego.PUERTA_SALIDA) {
						if(((Jugador)h.getObjetoJ(0)).getPiezaHabitacion()) {
							System.out.print(objJ.getLetraMapa());
						}else {
							if(col == 0 || col == Habitacion.ANCHO - 1) System.out.print("|"); //Paredes izquierda derecha
							if(fil == 0 || fil == Habitacion.ALTO - 1) System.out.print("="); //Paredes arriba abajo
						}
				...
				else if(col == 0 || col == Habitacion.ANCHO - 1) System.out.print("|"); //Paredes izquierda derecha
				else if(fil == 0 || fil == Habitacion.ALTO - 1) System.out.print("="); //Paredes arriba abajo
				else System.out.print(" "); //Mapa
				
				if(col == Habitacion.ANCHO) System.out.println(); //Salto de linea
			}
		}
		System.out.println("--------------------------------------------");
	}
	...
	public static int explotaHadron(int vidaPerdida) {
		...
		System.out.println("¡¡¡HAS CHOCADO CON UN HADRON!!!");
